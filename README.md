...
This does not change loading speed, does not auto load, and is not a cheat. It only adds a visual warning:

- When you finish a load but the server does not confirm a charge within a short grace window, the crossbow keeps its normal unloaded texture and the hotbar slot is tinted red.
- The red tint stays until that crossbow actually loads (becomes charged) or you swap it out.
- A normally unloaded crossbow that you have not tried to load is left with the default texture, no tint.
...
- Crossbow Ghost: the crossbow toggle and the green tint toggle.
...
The finished jar lands in `build/libs/fireghost-1.2.0.jar`. Drop it into your `mods` folder.
